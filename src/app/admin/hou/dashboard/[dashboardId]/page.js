...
//   const { tasks, loading, fetchStaffTasks, plans, staff } = usehouTask();
...
//   // useEffect(() => {
//   //   fetchStaffTasks("123", getCurrentMonth(), getCurrentWeekOfMonth());
//   // }, []);
...
//     const staffId = plans[0]?.user_id || "123"; // Fallback to "123" if no staff
//     fetchStaffTasks(staffId, month.label, activeWeek); // Use activeWeek as is (already in WEEK_X format)
//   };
...
//     const staffId = plans[0]?.user_id || "123"; // Fallback to "123" if no staff
//     fetchStaffTasks(staffId, selectedMonth, weekString); // send current month + new week
//   };
...
// the selected month is not showing staffId----------------------------------------------------------------------------------------
...
//       selectedMonth
//     );
//     fetchStaffTasks(staffId, selectedMonth, weekString); // Always fetch fresh data
//   };
...
//       activeWeek
//     );
//     fetchStaffTasks(staffId, selectedMonth, activeWeek);
...
import { useRouter } from "next/navigation";
import usehouTask from "../../../../../store/admin/usehouTask";
import usehouReport from "../../../../../store/admin/usehouReport";
...
export default function WeeklyPerformanceReview() {
  ...
  // const [selectedMonth, setSelectedMonth] = useState("August");
  const { tasks, loading, fetchStaffTasks, plans, staff } = usehouTask();
  const { fetchReport } = usehouReport();
  const monthDropdownRef = useRef(null);
  const isAnyDropdownOpen = isMonthOpen;
  const monthOptions = [
    { label: "JANUARY" },
  ...
  const handlePerformanceReview = async () => {
    try {
      ...
      const response = await fetchReport(staffId, selectedMonth, activeWeek);

      if (response.success) {
        // Navigate to performance review page after successful API call
        router.push(
          `/admin/hou/dashboard/${staffId}/performancereview/${staffId}?month=${selectedMonth}&week=${activeWeek}`
        );
      }
    } catch (error) {
