...
export default function WeeklyPerformanceReview() {
  ...
  const { tasks, loading, fetchStaffTasks, plans, staff } = useHosTask();
  ...
  const handleMonthSelect = (month) => {
    ...
    const departmentId = staff?.department_id; // Fallback department ID if not available
    const unit_id = staff?.unit_id;
    const ministryId = staff?.ministry_id; // Fallback ministry ID if not available
    ...
    fetchStaffTasks(
      ...
      month.label,
      activeWeek
    ); // Use activeWeek as is (already in WEEK_X format)
  };
  ...
  const handleWeekSelect = (week) => {
    ...
    const ministryId = staff?.ministry_id || "defaultMinistry"; // Fallback ministry ID if not available
