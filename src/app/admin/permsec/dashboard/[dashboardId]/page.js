    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-800 mb-2">
            {" "}
            <span className="capitalize">{departmentName}</span> Department
          </h1>
        </div>
        {/* Filter Section */}
        <div className="bg-white rounded-lg shadow-sm  mb-6">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <span
                        className={`inline-flex px-2 py-1 my-2 font-medium rounded-full ${
                          hods.plans?.[0]?.approved
                            ? " text-flag-green"
                            : " text-red-600"
                        }`}
                      >
                        {hods.plans?.[0]?.approved ? "Approved" : "Pending"}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <button
                        onClick={async () => {
                          const payload = {
                            staff_id: String(hods?.id),
                            department_id: String(hods?.department_id),
                            unit_id: String(hods?.unit_id),
                          ...
                          //  Send to backend
                          router.push(
                            `/admin/permsec/dashboard/${hods?.department_id}/workplan/${hods.id}`
                          ); //  Navigate
                        }}
