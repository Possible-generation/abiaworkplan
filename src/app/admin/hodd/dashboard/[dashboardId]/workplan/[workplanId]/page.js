...
export default function WeeklyPerformanceReview() {
  ...
  const [isMonthOpen, setIsMonthOpen] = useState(false);
  // const [selectedMonth, setSelectedMonth] = useState("August");
  const { tasks, loading, fetchStaffTasks, plans, staff } = usehodTask();
  ...
  const handleMonthSelect = (month) => {
    ...
    fetchStaffTasks(staffId, unitId, month.label, activeWeek); // Use activeWeek as is (already in WEEK_X format)
  };
  ...
  const handleWeekSelect = (week) => {
    ...
    fetchStaffTasks(staffId, unitId, selectedMonth, weekString); // Always fetch fresh data
  };
