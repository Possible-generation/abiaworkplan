...
//   const [hasTasksToReview, setHasTasksToReview] = useState(false); // NEW: Track if there are tasks
...
//         // If we have plans, try to populate tasks locally first
//         if (plans && plans.length > 0) {
//           console.log("Using existing plans to populate tasks");
//           populateTasksFromPlans();
//         } else {
//           console.log("No plans found, fetching plans first");
//           await fetchPlans();
//         }

//         // If still no tasks after trying local population, fetch from API
//         if (!tasks || tasks.length === 0) {
//           console.log("No tasks found locally, fetching from API");
//           await fetchTasks();
//         }
//       } catch (error) {
//         console.error("Error loading data:", error);
//       } finally {
//         setDataLoaded(true); // Mark data as loaded regardless of outcome
//       }
//     };
...
//   useEffect(() => {
//     if (tasks && tasks.length > 0 && selectedWeek && selectedMonth) {
//       // ✅ filter tasks to current week + month
//       const currentWeekTasks = tasks.filter(
//         (task) => task.week === selectedWeek && task.month === selectedMonth
//       );
...
//     }
//   }, [tasks, selectedWeek, selectedMonth]);
...
//   // MODIFIED: Auto-redirect to weekly report when all days are reviewed
//   useEffect(() => {
//     // NEW: Only proceed if data is loaded and we have tasks to review
//     if (!dataLoaded || !hasTasksToReview) {
//       return;
//     }

//     // Only check after we have both weekly tasks and completion status loaded
//     if (Object.keys(weeklyTasks).length > 0) {
//       const allDaysReviewed = DAYS.every((dayName) => {
//         const dayTasks = weeklyTasks[dayName] || [];
//         // If no tasks for this day, consider it reviewed
//         if (dayTasks.length === 0) return true;
//         // All tasks for this day must have completion status
...
//           console.log("All days reviewed, redirecting to weekly report");
...
//       }
//     }
//   }, [completionStatus, weeklyTasks, router, dataLoaded, hasTasksToReview]); // Added new dependencies
...
//             Reviewing tasks for {selectedWeek} of {selectedMonth}
//           </div>

//           <div className="flex justify-center items-center h-64">
//             <div className="text-center">
//               <div className="text-lg text-gray-600 mb-4">
//                 No tasks found for the selected week and month.
//               </div>
//               <div className="text-sm text-gray-500">
//                 Please create tasks first before reviewing them.
//               </div>
//               <button
//                 onClick={() => router.push("/")}
//                 className="mt-4 px-6 py-2 bg-flag-green text-white rounded-md hover:bg-flag-green"
//               >
//                 Go to Dashboard
//               </button>
//             </div>
...
//         {/* MODIFIED: Show completion message only when there are actually tasks reviewed */}
//         {(() => {
...
//                   className={`flex-1 py-2 px-4 rounded-md text-sm font-medium ${
//                     reason.trim() && !loading
//                       ? "bg-flag-green text-white hover:bg-flag-green-dark"
//                       : "bg-gray-300 text-gray-500 cursor-not-allowed"
//                   }`}
//                 >
//                   {loading ? "Saving..." : "Send"}
...
//               </div>
//               <button
//                 onClick={() => router.push("/weeklyreport")}
//                 className="ml-6 px-6 py-3 bg-green-600 text-white rounded-md hover:bg-green-700 font-medium whitespace-nowrap"
//               >
//                 View Weekly Report
//               </button>
//             </div>
